...
{
    check( to != get_self(), "Donation not accepted. To sell that token use selltoken.");
...
int64_t evolutiondex::compute(int64_t x, int64_t y, int64_t z, int fee) {
    ...
    eosio::check( (tmp <= MAX), "computation overflow" );
...
void evolutiondex::add_or_remove(name user, asset to_buy, bool is_buying){
    ...
    auto to_pay1 = compute(to_buy.amount, C1, A, fee);
    auto to_pay2 = compute(to_buy.amount, C2, A, fee);
    
    add_balance(user, extended_asset{ asset{-to_pay1, token->connector1.quantity.symbol}, token->connector1.contract});
    add_balance(user, extended_asset{ asset{-to_pay2, token->connector2.quantity.symbol}, token->connector2.contract});
...
void evolutiondex::add_balance( const name& user, const extended_asset& to_add )  // cambiar nombre por add_signed_balance
{
    ...
    index.modify( acnt_balance, ""_n, [&]( auto& a ) {  // puede pasar que consuma más ram?
        ...
        check( a.balance.quantity.amount <= MAX, "balance cannot be larger than 2^64-1");
